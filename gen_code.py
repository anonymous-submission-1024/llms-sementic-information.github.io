...
def get_directory_structure(path, prefix=''):
    ...
    for item in os.listdir(path):
        ...
        if os.path.isdir(item_path):
            if item not in ['venv', '__pycache__', '.git', 'dist', 'node_modules', 'gen_code.py']:
...
def get_all_files(path):
    ...
    for root, _, files in os.walk(path):
        if any(excluded_dir in root for excluded_dir in ['venv', '__pycache__', '.git','dist', 'node_modules', 'gen_code.py']):
            continue
